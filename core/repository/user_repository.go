...
func (p *pgUserRepository) SearchUser(ctx context.Context, id string, q string) (res []user.UserShortInfo, err error) {
	log.Println(q)
	query := `select client_id,nombre,is_admin,(false),email,profile_photo,estado 
...
func (p *pgUserRepository) ValidateEmail(ctx context.Context, m string) (err error) {
	...
	err = p.Conn.QueryRow(ctx, query, m).Scan(&email)
	if email == m {
		return errors.New(fmt.Sprintf("Ya existe un usuario con este email: %s", email))
	}
	query1 := `select email from invitaciones where email = $1;`
	err = p.Conn.QueryRow(ctx, query1, m).Scan(&email)
	if email == m {
		return errors.New(fmt.Sprintf("Ya existe una invitacion con este email: %s", email))
	}
	return nil
}

func (p *pgUserRepository) GetUsersShortIInfo(ctx context.Context, id string) (res []user.UserShortInfo, err error) {
	query := `select client_id,CONCAT  (nombre, ' ', apellido) AS "Full name",is_admin,(false),email,profile_photo,estado from clientes where superior_id = $1`
	res, err = p.fetchUserShortInfo(ctx, query, id)
	if err != nil {
		return nil, err
	}
	// list2,err := p.GetInvitaciones(ctx,id)
	// if err != nil{
	// 	return nil,err
	// }
...
func (p *pgUserRepository) GetInvitaciones(ctx context.Context, id string) (res []user.UserShortInfo, err error) {
	query := `select id,email,is_admin,pendiente,(''),(''),(0) from invitaciones where creador_id = $1`
	res, err = p.fetchUserShortInfo(ctx, query, id)
	if err != nil {
		return nil, err
	}
...
func (p *pgUserRepository) CreateUserInvitation(ctx context.Context, us *user.UserShortInfo) (res user.UserShortInfo, err error) {
	...
	query2 := `insert into invitaciones (email,is_admin,creador_id) values($1,$2,$3) returning (id,email,pendiente,is_admin,(''),(0));`
	t := user.UserShortInfo{}
	err = p.Conn.QueryRow(ctx, query2, us.Nombre, us.IsAdmin, superiorId).Scan(&t)
	if err != nil {
		return user.UserShortInfo{}, err
	}
	return t, nil
...
func (p *pgUserRepository) GetClientesFiltered(ctx context.Context, f int) (res []user.UserArea, err error) {
	// log.Println(f)
	query2 := `select user_id,nombre_user,(0) from user_area where area_id = $1;`
	res2, _ := p.fetchUserArea(ctx, query2, f)
	log.Println("len res2", len(res2))

	query := `select client_id,nombre,(0) from clientes;`
	res, err = p.fetchUserArea(ctx, query)
	if len(res2) == 0 {
		return res, nil
	} else {
		users:= res
...
func (p *pgUserRepository) GetClienteById(ctx context.Context, id string) (res user.Cliente, err error) {
