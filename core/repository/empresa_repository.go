...
func(p *pgEmpresaRepository) GetUsersAreaByAreaId(ctx context.Context,areaId int)(res []empresa.UserArea,err error){
...
func (p *pgEmpresaRepository) GetSubAreasFuncionario(ctx context.Context,emId int)(res []empresa.Area,err error){
	query := `select id,nombre,estado,empresa_id,created_on,creador_id from sub_areas where empresa_parent_id = $1`
...
func (p *pgEmpresaRepository) GetSubAreasEmpresa(ctx context.Context,emId int)(res []empresa.Area,err error){
	query := `select id,nombre,estado,empresa_id,created_on,creador_id from sub_areas where empresa_id = $1`
	res,err = p.fetchAreas(ctx,query,emId)
	if err != nil {
		return 
	}
...
func (p *pgEmpresaRepository) GetSubAreas(ctx context.Context,parentId int)(res []empresa.Area,err error){
	query := `select id,nombre,estado,empresa_id,created_on,creador_id from sub_areas where parent_id = $1`
...
func (p *pgEmpresaRepository) CreateSubArea(ctx context.Context,a *empresa.SubArea)(err error){
	list,err := p.GetSubAreasEmpresa(ctx,a.EmpresaId)
	if err != nil{
		return
	}
	...
	var areaId int
	query1 := `insert into sub_areas (nombre,empresa_id,created_on,creador_id,empresa_parent_id,parent_id)
	values ($1,$2,$3,$4,$5,$6) returning (id)`
	err = p.Conn.QueryRowContext(ctx,query1,a.Nombre,a.EmpresaId,time.Now(),a.CreadorId,a.EmpresaParentId,a.ParentId).Scan(&areaId)
